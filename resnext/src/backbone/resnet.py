...
class ResNet(nn.Cell):
    ...
    def __init__(self, block, layers, width_per_group=64, groups=1, use_se=False, platform="Ascend"):
        ...
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2, use_se=use_se, platform=platform)    # 1111111111

        self.out_channels = 512 * block.expansion             # 2222222222222222
        self.cast = P.Cast()
    ...
    def construct(self, x):
        ...
        x = self.layer2(x)
        x = self.layer3(x)
        x = self.layer4(x)                     # 33333333333333333333
