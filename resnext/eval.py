...
'''
def tta_augment(image, c, b):                          # zengzengzengzengzeng
    b, ch, h, w = image.shape
    image = image.reshape(ch, h, w).transpose((1, 2, 0))
    blank = np.zeros_like(image, image.dtype)
    dst = cv.addWeighted(image, c, blank, 1-c, b)
    dst = np.expand_dims(dst.transpose((2, 0, 1)), axis=0)
    image = np.expand_dims(image.transpose((2, 0, 1)), axis=0)
    image = np.concatenate((dst, image), axis=0)
    return image
'''
def tta_flip(image):                          # zengzengzengzengzeng
    b, ch, h, w = image.shape
    image = image.reshape(ch, h, w).transpose((1, 2, 0))
    dst = cv.flip(image, 1)
    region2 = image[12:244, 12:244, :]
    dst2 = cv.resize(region2, (256, 256))
    region3 = image[24:232, 24:232, :]
    dst3 = cv.resize(region3, (256, 256))
    region4 = image[36:220, 36:220, :]
    dst4 = cv.resize(region4, (256, 256))
    region5 = image[48:208, 48:208, :]
    dst5 = cv.resize(region5, (256, 256))
    
    dst = np.expand_dims(dst.transpose((2, 0, 1)), axis=0)
    dsta = dst
    dstb = dst
    dst2 = np.expand_dims(dst2.transpose((2, 0, 1)), axis=0)
    dst3 = np.expand_dims(dst3.transpose((2, 0, 1)), axis=0)
    dst4 = np.expand_dims(dst4.transpose((2, 0, 1)), axis=0)
    dst5 = np.expand_dims(dst5.transpose((2, 0, 1)), axis=0)
    image = np.expand_dims(image.transpose((2, 0, 1)), axis=0)
    
    image = np.concatenate((image, dst, dst2, dst3, dst4, dst5), axis=0)
    #image = np.concatenate((image, dst), axis=0)
    return image
        
@moxing_wrapper()
def test():
    """test"""
    datafile = os.path.join('.', 'data_second.csv')
    set_parameters()
    ...
    for model in config.models:
        ...
        with open (datafile, 'w') as f:
            for data, gt_classes in eval_dataloader:
                ...
                output = np.sum(output, 0, keepdims = True)                          # zengzengzengzengzeng
                #print('sss', output.shape)

                top1_output = np.argmax(output, (-1))
                '''
                if output[0][top1_output] < 55:
                    top1_output = np.array([21])
                    k = 1
                if top1_output > 20 and k == 0:
                    top1_output += 1
                '''
                #config.logger.info('number is {}' 'index is: {}'' confidence is: {}' .format(xuhao, top1_output, output[0][top1_output]))
                #if output[0][top1_output] < minnumber:
                #    minnumber = output[0][top1_output]
                #config.logger.info(output[0][top1_output])
                top5_output = np.argsort(output)[:, -5:]
                
                #if top1_output != gt_classes:
                    #f.write(str(top1_output))
                    #f.write(',')
                    #f.write(str(gt_classes))
                    #f.write('\n')
        ...
        #config.logger.info('min number is: {}'.format(minnumber))
    if config.run_distribute:
        release()
